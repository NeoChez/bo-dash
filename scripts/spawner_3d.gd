...
# --- Obstacle scenes (user punya, dari scenes/obstacle/) ---
...
@export var chalwall_delay: float = 2
...
func _on_spawn_timer() -> void:
	if not _match_active or not obstacles_enabled:
		return

	var roll = randi() % 4
	if roll == 0:
		print(offset_obstacle1)
...
func _spawn(scene: PackedScene, offset: Vector3 = Vector3.ZERO) -> void:
	...
	print("global pos before: ", obs.global_position.y)
	if ground_node:
		obs.global_position.y = ground_node.global_position.y + 1 + offset.y
		print("hasil perhitungan: ", ground_node.global_position.y + 1 + offset.y)
		print("global pos after: ", obs.global_position.y)
	var direction = Vector3(arah_x, 0, 0)
	if obs.has_method("set_movement"):
		obs.set_movement(direction, move_speed)
	else:
...
func _on_star_timer() -> void:
	if not _match_active or star_scene == null:
		return
...
func _on_skill_timer() -> void:
	if not _match_active or skill_item_scene == null:
		skill_timer.wait_time = skill_spawn_interval
		skill_timer.start()
		return
...
func _spawn_skill() -> void:
	...
	add_child(item)
	item.top_level = true
	item.global_transform = $Lane.global_transform
	if wrap_src != null:
